    ...
    */
	...
	while ($row = mysqli_fetch_array($salida)) {
		...
		$tablaHTML .= '<div class="span-9" >';
            ...
            if ($conBorrar) {
                $tablaHTML .= '<div class="span-1 last cursorPoint fila_impar"  ><input type="button" class="enviarpeqRojo" onclick="borrarReg('.$jsdel.')" value="'.$borrar.'" style="width:50px; "/></div>';
            }
        $tablaHTML .= '</div>';
	}
	$tablaHTML .= '</div>';
