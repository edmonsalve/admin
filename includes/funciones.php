<?php
    ...
    function ipcSerieEmpalmada($aaaaCAL,$mmCAL){
        $ipcAcumulado       = 0;
        $ipcAcumulado_a2023 = 0;
        $aaaaAct            = date('Y');
        $mmAct              = date('m');
        ...
        $consultaDesde = "SELECT * FROM `comun_tabIPCEmpalmada` WHERE `aaaa` = $aaaaCAL"; 
        $salida        = $conexionIPC->consulta($consultaDesde);
        ...
        // :::::  Lee Puntos Finales :::::::::::: //
        $consultaHasta   = "SELECT * FROM `comun_tabIPCEmpalmada` WHERE `aaaa` = $aaaaHasta"; 
        
        $salida     = $conexionIPC->consulta($consultaHasta);
        $rowIPC     = mysqli_fetch_array($salida,MYSQLI_ASSOC);
        $puntosFin  = $rowIPC['ipc_'.$mesFinCampo];

        $ipcAcumulado = (($puntosFin / $puntosIni) - 1) * 100; 
        // echo "<br>ipcSerieEmpalmada<br>ipc_$mesFinCampo<br> ==> $ipcAcumulado = (($puntosFin / $puntosIni) - 1) * 100";
    ...
    function calcularIpcGeneral($aaaaCAL,$mmCAL) { 
        ...
        // - 2;  Nunca sera Negativo el Primer mes de pago es Marzo (3-2 = 1 Toma ENE)
        ...
        $mesIniv      = $mesIniCampo; 
        $aaaaCALSerie = $aaaaCAL;

        if($aaaaCAL < 2010) {
            $consultaIPC    = "SELECT * FROM `comun_tabUtm` WHERE `aaaa` = $aaaaCALSerie"; 
            $salida         = $conexionIPC->consulta($consultaIPC);
            $rowIPC         = mysqli_fetch_array($salida,MYSQLI_ASSOC);
            $puntosIni2009  = $rowIPC['ipc_'.$mesIniCampo];
            mysqli_free_result($salida);  
            
            $consulta       = "SELECT * FROM `comun_tabUtm` WHERE `aaaa` = 2009"; 
            $salida         = $conexionIPC->consulta($consulta);
            $rowIPC         = mysqli_fetch_array($salida,MYSQLI_ASSOC);
            $puntosFin2009  = 98.62;  // IPC POR PUNTOS DIC 2009
            mysqli_free_result($salida);     
            
            $ipcAcumulado_a2009 = (($puntosFin2009 / $puntosIni2009) *100) - 100;
        ...
        if ($aaaaCAL < 2024) {
            ...
            $salida         = $conexionIPC->consulta($consultaIPC);
            $rowIPC         = mysqli_fetch_array($salida,MYSQLI_ASSOC);
            $puntosIni2023  = $rowIPC['ipc_'.$mesIniCampo];
            mysqli_free_result($salida);

            // :::::  Lee Puntos Finales :::::::::::: //
            $consulta       = "SELECT * FROM `comun_tabUtm` WHERE `aaaa` = 2023"; 
            $salida         = $conexionIPC->consulta($consulta);
            $rowIPC         = mysqli_fetch_array($salida,MYSQLI_ASSOC);
            $puntosFin2023  = 134.10; // IPC POR PUNTOS DIC 2023
            mysqli_free_result($salida);

            $ipcAcumulado_a2023 = (($puntosFin2023 / $puntosIni2023) *100) - 100;
            if ($ipcAcumulado_a2023 < 0) { $ipcAcumulado_a2023 = 0; }
            $aaaaCALSerie = 2024;
        }
        ...
        if ($aaaaAct >= 2024) {
            ...
            if ($aaaaCALSerie == 2024 ) { $puntosIni = 101.72; } else { $puntosIni = $rowIPC['ipc_'.$mesIniCampo]; }
            mysqli_free_result($salida);
            ...
            if ($aaaaCAL > 0) {
                ...
                // echo "<br>calcularIpcGeneral<br>ipc_$mesFinCampo<br> ==> $ipcAcumulado = (($puntosFin / $puntosIni) * 100) - 100"; 
                $ipcAcumulado = (($puntosFin / $puntosIni) * 100) - 100; 

                //echo "<br>MF $mesFinCampo -- $ipcAcumulado = (($puntosFin / $puntosIni) * 100) - 100"; 
                //echo "<br>SERIE: $aaaaCALSerie || $aaaaCAL, $mmCAL || *** MesIni: $mesIni ** MesFin: $mesFin  *** <br>";  
            }
        }

        if ($ipcAcumulado < 0) { $ipcAcumulado = 0; }
        $ipcAcumulado  += $ipcAcumulado_a2009 + $ipcAcumulado_a2023;
        $ipc            = round($ipcAcumulado,1); 

        /* 
        echo "$aaaaCAL, $mmCAL<br>";
        echo "$ipcAcumulado = (($puntosFin / $puntosIni) *100) - 100;  <br>";
        echo "$aaaaCAL, $mmCAL || *** MesIni: $mesIni ** MesFin: $mesFin  *** <br>";  


        echo "PI 2009: $puntosIni2009 - PF 2009: $puntosFin2009  <br> ";
        echo "PI 2023: $puntosIni2023 - PF 2023: $puntosFin2023  <br> ";
        echo "PI NEW0: $puntosIni - PF NEW0: $puntosFin  <br> ";
        echo "IPC Acum 2009: $ipcAcumulado_a2009 -- IPC2023: $ipcAcumulado_a2023  -- IPC TOT: $ipc <br>";
        echo "==================================================================<br>";
        */   
        return($ipc);  
    }
    ...
    function bieniosN($fecha_ing,$mes,$ano) {
        $ano_ing = substr($fecha_ing,0,4);
        ...
        $years   = $ano - $ano_ing;        
        if ($mes_ing < $mes) {
            $bienio = intval($years / 2);
            } else {
            if ((($ano - $ano_ing) % 2) == 0) {
                $bienio = intval(($ano - $ano_ing) / 2) - 1 ;
                } else {
                $bienio = intval(($ano - $ano_ing) / 2) ;
            }
        }
            
        if 	($bienio < 0) 	{ $bienio = 0; }
        if 	($bienio > 15) 	{ $bienio = 15; }
        return ($bienio);
    }
?>